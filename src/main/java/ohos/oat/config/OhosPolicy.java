 ...
 * @since 1.0
 */
public class OhosPolicy {
    private final Map<String, List<OhosPolicyItem>> policyItemListMap = new HashMap<>();

    private String namne;

    private String desc;

    public List<OhosPolicyItem> getPolicyItems(final String policyType) {
        List<OhosPolicyItem> policyItemList = this.policyItemListMap.get(policyType);
    ...
    public void addPolicyItem(final OhosPolicyItem ohosPolicyItem) {
        final String policyType = ohosPolicyItem.getType();
        List<OhosPolicyItem> policyItemList = this.policyItemListMap.get(policyType);
        if (null == policyItemList) {
            policyItemList = new ArrayList<>();
            this.policyItemListMap.put(policyType, policyItemList);
        }
        policyItemList.add(ohosPolicyItem);
    }

    public List<OhosPolicyItem> getAllPolicyItems() {
        final List<OhosPolicyItem> allPolicyItems = new ArrayList<>();
        for (final Map.Entry<String, List<OhosPolicyItem>> stringListEntry : this.policyItemListMap.entrySet()) {
