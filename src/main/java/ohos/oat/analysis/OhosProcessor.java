...
import ohos.oat.analysis.headermatcher.IOhosHeaderMatcher;
import ohos.oat.analysis.headermatcher.OhosCopyrightMatcher;
import ohos.oat.analysis.headermatcher.OhosCustomizedTextLicenseMatcher;
...
import ohos.oat.analysis.headermatcher.OhosSpdxLabelLicenseMatcher;
import ohos.oat.analysis.headermatcher.OhosSpdxTextLicenseExceptionMatcher;
import ohos.oat.analysis.headermatcher.OhosSpdxTextLicenseMatcher;
...
public class OhosProcessor {
    ...
    private final List<IOhosHeaderMatcher> defaultHeaderMatchers_ori = new ArrayList<>();

    private final List<IOhosHeaderMatcher> defaultHeaderMatchers_clean = new ArrayList<>();

    private final List<IOhosHeaderMatcher> defaultHeaderMatchers_exception_clean = new ArrayList<>();
    ...
    public OhosProcessor(final Reader fileReader, final Document fileDocument, final OhosConfig ohosConfig) {
        ...
        if (fileDocument instanceof OhosFileDocument) {
            ...
            OhosLogUtil.error(this.getClass().getSimpleName(),
                "Document invalid: " + fileDocument.getClass().getName());
        }
    ...
    private void initDefaultMatchers() {
        ...
        this.defaultHeaderMatchers_ori.add(new OhosCopyrightMatcher());
        this.defaultHeaderMatchers_ori.add(new OhosImportMatcher());
        this.defaultHeaderMatchers_ori.add(new InvalidLicense());
        this.defaultHeaderMatchers_ori.add(new OhosSpdxLabelLicenseMatcher());
        ...
        for (final OhosLicense ohosLicense : spdxLicenseList) {
            this.defaultHeaderMatchers_clean.add(
                new OhosSpdxTextLicenseMatcher(ohosLicense.getLicenseId(), ohosLicense.getLicenseHeaderText(),
                    ohosLicense.getUrls()));
        }
        final List<OhosLicense> spdxLicenseExceptionList = this.ohosConfig.getExceptionLicenseList();
        for (final OhosLicense ohosLicense : spdxLicenseExceptionList) {
            this.defaultHeaderMatchers_exception_clean.add(
                new OhosSpdxTextLicenseExceptionMatcher(ohosLicense.getLicenseId(), ohosLicense.getLicenseHeaderText(),
                    ohosLicense.getUrls()));
        }
    ...
    private boolean readLine() throws IOException {
        ...
        if (line.trim().length() > 0) {
            ...
            final String cleanStr = OhosLicenseTextUtil.cleanAndLowerCaseLetter(line);
