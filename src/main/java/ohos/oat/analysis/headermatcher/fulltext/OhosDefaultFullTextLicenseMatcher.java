...
import ohos.oat.analysis.headermatcher.OhosFullTextLicenseMatcher;
 ...
 * @since 1.0
 */
public class OhosDefaultFullTextLicenseMatcher extends OhosFullTextLicenseMatcher {
    protected OhosDefaultFullTextLicenseMatcher(final MetaData.Datum licenseFamilyCategory,
        final MetaData.Datum licenseFamilyName, final String notes, final String fullText) {
        super(licenseFamilyCategory, licenseFamilyName, notes, OhosLicenseTextUtil.cleanAndLowerCaseLetter(fullText));
    }
    ...
    public boolean match(final Document subject, final String line) throws RatHeaderAnalysisException {
        ...
        if (OhosMatchUtils.needMatchAgain(licenseName, this.getLicenseFamilyName())) {// copyleft need match again
