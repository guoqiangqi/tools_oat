...
/**
 * Licensed under the license text matcher
 *
 * @author chenyaxun
 * @since 1.0
 */
public class UnderLicenseMatcher extends AbstractOatLicenseMatcher {
    private final Pattern pattern = IOatMatcher.compilePattern("(Licensed under the (.*))");

    private int line;

    public UnderLicenseMatcher() {
    }
    ...
    public boolean match(final IOatDocument pSubject, final String licenseTxt) {
        ...
        final int index = licensHeaderText.indexOf("Licensed under the");
