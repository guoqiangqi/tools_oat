...
```
批量模式生成的报告位于OAT的运行目录，其中OAT-ALL.xml用于配置待扫描的项目清单及默认的许可证、Copyright等策略及默认的过滤规则。
...
8、policyitem desc：用于描述该策略的原因，如许可证类型为GPL V2，在desc字段要描述为什么要用此许可证，被什么特性使用，是否是跨进制通信等。
